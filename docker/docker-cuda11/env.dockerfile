FROM harbor.lightwheel.net/lwad/env/cuda:11.8.0-devel-ubuntu22.04
...
# for python 3.10
RUN apt update
RUN DEBIAN_FRONTEND=noninteractive TZ=Asia/Shanghai apt install curl zsh sudo wget python3-full python3-dev python3-pip net-tools -y
...
RUN https_proxy=http://10.10.0.78:7890 http_proxy=http://10.10.0.78:7890 all_proxy=socks5://10.10.0.78:7890 pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118 && pip cache purge
RUN pip install ninja cython addict packaging Pillow matplotlib prettytable six terminaltables lyft_dataset_sdk scikit-image tensorboard cityscapesscripts imagecorruptions scipy scikit-learn networkx ipython opencv-python==4.8.0.74 seaborn einops casadi torchmetrics trimesh pytest pytest-cov pytest-runner flake8 similaritymeasures py-trees simple_watchdog_timer transforms3d tabulate ephem dictor open3d && pip cache purge

# Specific versions and conditions
RUN apt install cargo -y && pip install numba motmetrics yapf==0.40.1 trimesh laspy lazrs torch_scatter deepspeed && pip cache purge

# Platform-specific packages
RUN pip install "regex; sys_platform=='win32'" "pycocotools; platform_system == 'Linux'" "pycocotools-windows; platform_system == 'Windows'" && pip cache purge
...
RUN printf '#!/bin/bash\n/usr/sbin/sshd\nzsh' > /.start.sh && chmod +x /.start.sh

ARG CUDA_VERSION=11.8.0
ARG CUDA_ARCHITECTURES=9.0;8.9;8.6;8.0
ENV TORCH_CUDA_ARCH_LIST="${CUDA_ARCHITECTURES}+PTX"
